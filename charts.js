...
const Charts = (() => {
  ...
  /** Gráfico de barras verticais com cor individual por item — comparação entre metas. */
  function comparisonBars(canvas, items) {
    const { ctx, width, height } = setupCanvas(canvas);
    if (!items.length) {
      emptyState(ctx, width, height, "Cadastre metas para comparar");
      return;
    }
    ...
    const maxV = Math.max(...items.map((i) => i.value), 1);
    const gap = 10;
    const barW = Math.max((plotW - gap * (items.length - 1)) / items.length, 6);

    items.forEach((item, i) => {
      const x = padding.left + i * (barW + gap);
      const barHpx = Math.max((item.value / maxV) * plotH, 2);
      const y = padding.top + plotH - barHpx;
      ctx.fillStyle = item.color;
      roundRect(ctx, x, y, barW, barHpx, 5);
      ctx.fill();
  ...
  return { lineChart, donutChart, progressBars, barChart, comparisonBars };
})();
