...
- **Seu Momento**: card no Painel com uma mensagem motivacional calculada automaticamente a partir do progresso da meta principal (0%, 10%, 25%, 50%, 75%, 90% e 100%).
...
- **Sistema de conquistas (badges)** organizado por categoria (marcos de progresso, valor acumulado, constância e metas concluídas), desbloqueadas automaticamente conforme o uso.
- **Análises**: gráficos de evolução acumulada, distribuição por meta, progresso de cada meta, comparação entre metas e aportes por mês — desenhados em `<canvas>`, sem depender de bibliotecas externas (funcionam 100% offline).
- **Histórico geral** de todos os aportes, com ícone da meta, valor destacado e data/hora ("Hoje • 14:32"), além de filtro por meta e por categoria.
...
- **Animações discretas**: cartões surgem suavemente, barras de progresso preenchem ao carregar, números fazem uma contagem suave, botões respondem ao toque — tudo otimizado para continuar leve e rápido.
...
Todos os arquivos ficam na raiz do repositório, e todos os caminhos usados no projeto são **relativos** a ela (`./style.css`, `./app.js`, etc.), então o app funciona tanto na raiz de um domínio quanto em um subdiretório (como é o caso do GitHub Pages em repositórios de projeto: `usuario.github.io/repositorio/`).
...
**Foto do objetivo (cache `metas-v4`)**
- Cada meta agora pode ter uma foto opcional, escolhida da galeria do celular ou do computador.
- A foto é redimensionada e comprimida automaticamente (JPEG, até 900px) antes de ser salva, para não estourar o espaço do `localStorage`.
- A foto aparece no cartão da meta, no modal de detalhes, no card "Foco atual" (quando é a meta principal) e é preservada quando a meta é concluída e movida para "Conquistas".
- A imagem é incluída automaticamente na exportação/importação de backup (arquivo `.json`).
- Metas antigas (sem foto) continuam funcionando normalmente — nenhuma migração manual é necessária.
- Nenhum upload é feito para servidor: a imagem fica salva apenas no dispositivo do usuário.

**Atualização visual (cache `metas-v3`)**
- Novo card "🔥 Seu Momento" no Painel, com mensagem motivacional automática.
- Barras de progresso e números com animação suave ao carregar a tela.
- Cartões com entrada animada e discreta em todas as telas.
- Histórico redesenhado com ícone, sinal "+" e data/hora.
- Tela de Conquistas reorganizada por categoria, com 5 novos badges de marco de progresso (10/25/50/75/100%).
- Novo gráfico "Comparação entre metas" na tela de Análises.
- Botões, campos de formulário e modais ajustados para uma experiência mais confortável no iPhone (sem zoom automático em campos, rolagem nativa, área de toque maior).
- Nenhuma alteração na lógica de armazenamento, metas, contribuições ou histórico — todos os dados existentes continuam funcionando normalmente.
