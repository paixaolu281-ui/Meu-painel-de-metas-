...
const DB = (() => {
  ...
  function load() {
    try {
      ...
      // Migração compatível: metas salvas antes do recurso de foto não têm
      // "imageData" — garantimos o campo (null) para que o restante do app
      // possa sempre checar `goal.imageData` sem precisar tratar undefined.
      merged.goals = (merged.goals || []).map((g) => ({ imageData: null, ...g }));
      return merged;
    } catch (err) {
  ...
  function persistAndNotify() {
    lastPersistOk = save(state);
    notify();
    return lastPersistOk;
  }

  /** Indica se a última gravação no localStorage foi bem-sucedida (ex: false quando o dispositivo está sem espaço). */
  function wasLastSaveSuccessful() {
    return lastPersistOk;
  }
  ...
  const ACHIEVEMENT_DEFS = [
    // Progresso — marcos de porcentagem alcançados em qualquer meta individual
    { id: "progress-10", icon: "🎯", label: "10% de uma meta", desc: "Alcance 10% de progresso em qualquer meta.", group: "progresso" },
    { id: "progress-25", icon: "🔥", label: "25% de uma meta", desc: "Alcance 25% de progresso em qualquer meta.", group: "progresso" },
    { id: "progress-50", icon: "💪", label: "Metade do caminho", desc: "Alcance 50% de progresso em qualquer meta.", group: "progresso" },
    { id: "progress-75", icon: "🚀", label: "75% de uma meta", desc: "Alcance 75% de progresso em qualquer meta.", group: "progresso" },
    { id: "progress-100", icon: "🏆", label: "100% de uma meta", desc: "Leve qualquer meta até a conclusão.", group: "progresso" },

    // Valor acumulado
    { id: "first-100", icon: "🏅", label: "Primeiros R$ 100", desc: "Acumule R$ 100 no total.", group: "valor" },
    { id: "first-500", icon: "🏅", label: "Primeiros R$ 500", desc: "Acumule R$ 500 no total.", group: "valor" },
    { id: "first-1000", icon: "🏅", label: "Primeiros R$ 1.000", desc: "Acumule R$ 1.000 no total.", group: "valor" },
    { id: "acc-5000", icon: "💎", label: "R$ 5.000 acumulados", desc: "Acumule R$ 5.000 no total.", group: "valor" },
    { id: "acc-10000", icon: "💎", label: "R$ 10.000 acumulados", desc: "Acumule R$ 10.000 no total.", group: "valor" },

    // Constância
    { id: "streak-7", icon: "🔥", label: "7 dias economizando", desc: "Registre aportes em 7 dias diferentes.", group: "constancia" },
    { id: "streak-30", icon: "🔥", label: "30 dias economizando", desc: "Registre aportes em 30 dias diferentes.", group: "constancia" },

    // Metas concluídas
    { id: "first-goal-done", icon: "🎯", label: "Primeira meta concluída", desc: "Conclua sua primeira meta.", group: "metas" },
    { id: "goals-3", icon: "🏆", label: "3 metas concluídas", desc: "Conclua 3 metas.", group: "metas" },
    { id: "goals-5", icon: "👑", label: "5 metas concluídas", desc: "Conclua 5 metas.", group: "metas" },
  ];
  ...
  function evaluateAchievements() {
    ...
    const maxGoalProgress = state.goals.reduce((max, g) => {
      if (!g.targetAmount || g.targetAmount <= 0) return max;
    ...
    tryUnlock("progress-10", maxGoalProgress >= 10);
    tryUnlock("progress-25", maxGoalProgress >= 25);
    tryUnlock("progress-50", maxGoalProgress >= 50);
    tryUnlock("progress-75", maxGoalProgress >= 75);
    tryUnlock("progress-100", maxGoalProgress >= 100);
  ...
  function importData(json) {
    ...
    // Compatível com backups antigos (sem foto) e novos (com foto em base64).
    state.goals = state.goals.map((g) => ({ imageData: null, ...g }));
    persistAndNotify();
  }
