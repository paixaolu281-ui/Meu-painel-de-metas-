...
(() => {
  ...
  /** "Hoje • 14:32", "Ontem • 09:10" ou "05/09 • 09:10" — usado nos itens de histórico. */
  function formatEntryMeta(isoOrDateStr) {
    const d = new Date(isoOrDateStr);
    if (isNaN(d)) return "";
    ...
    if (dOnly.getTime() === today.getTime()) return `Hoje • ${time}`;
    if (dOnly.getTime() === yesterday.getTime()) return `Ontem • ${time}`;
    return `${formatDateShort(isoOrDateStr)} • ${time}`;
  }
  ...
  // ---------------------------------------------------------------------
  // Foto do objetivo — leitura e compressão de imagem
  // ---------------------------------------------------------------------

  const PHOTO_MAX_DIMENSION = 900; // px — suficiente para cartões e telas de detalhe
  const PHOTO_JPEG_QUALITY = 0.72;

  /** HTML da área de foto do formulário: prévia da imagem ou botão para adicionar. */
  function photoPreviewMarkup(dataUrl) {
    if (dataUrl) {
      return `
        <div class="photo-preview">
          <img src="${dataUrl}" alt="Prévia da foto do objetivo" />
          <div class="photo-preview-actions">
            <button type="button" class="btn btn-secondary btn-sm" id="photo-change-btn">Trocar foto</button>
            <button type="button" class="btn btn-danger btn-sm" id="photo-remove-btn">Remover</button>
          </div>
        </div>`;
    }
    return `
      <button type="button" class="photo-placeholder-btn" id="photo-add-btn">
        <span class="photo-placeholder-icon">📸</span>
        <span>Adicionar foto</span>
      </button>`;
  }

  /** Liga os eventos dos botões da área de foto (adicionar / trocar / remover). */
  function bindPhotoUpload() {
    const fileInput = qs("#f-photo-input");
    const uploadBox = qs("#photo-upload");

    const openPicker = () => fileInput.click();
    qs("#photo-add-btn")?.addEventListener("click", openPicker);
    qs("#photo-change-btn")?.addEventListener("click", openPicker);
    qs("#photo-remove-btn")?.addEventListener("click", () => {
      pendingPhotoData = null;
      uploadBox.innerHTML = photoPreviewMarkup(null);
      bindPhotoUpload();
    });
    ...
    fileInput.onchange = async () => {
      ...
      if (!file) return;
      try {
        pendingPhotoData = await readAndCompressImage(file);
        uploadBox.innerHTML = photoPreviewMarkup(pendingPhotoData);
        bindPhotoUpload();
      } catch (err) {
        console.error(err);
   ...
   * proporção) e comprime em JPEG, retornando uma data URL base64 pronta
   * para ser guardada junto com a meta no localStorage. Isso evita salvar
   * fotos de câmera em resolução total (vários MB), o que rapidamente
   * estouraria a cota de armazenamento do navegador.
   */
  ...
  let pendingPhotoData = null; // foto selecionada (ainda não salva) no formulário de meta aberto
  ...
  /** Mensagem automática do card "Seu Momento", calculada a partir do progresso (0-100). */
  function momentumMessage(p) {
    if (p >= 100) return { icon: "🎉", text: "Meta concluída! Parabéns pela conquista!" };
    if (p >= 90) return { icon: "🔥", text: "Última reta! Sua meta está muito perto." };
    if (p >= 75) return { icon: "🏆", text: "Falta pouco! Você está chegando lá." };
    if (p >= 50) return { icon: "🚀", text: "Metade do caminho já foi conquistada!" };
    if (p >= 25) return { icon: "💪", text: `Você já está em ${p.toFixed(0)}% da sua meta.` };
    if (p >= 10) return { icon: "🔥", text: "Você já começou a construir seu objetivo." };
    return { icon: "🚀", text: "O primeiro passo já foi dado. Agora é continuar." };
  }
  ...
  function renderPainel() {
    ...
    if (main) {
      ...
      focusHtml = `
        <div class="focus-card card-enter ${main.imageData ? "has-photo" : ""}">
          ${main.imageData ? `<div class="focus-photo"><img src="${main.imageData}" alt="Foto do objetivo: ${escapeHtml(main.name)}" /></div>` : ""}
          <div class="focus-card-body">
          <div class="focus-eyebrow">🎯 Foco atual</div>
          <div class="focus-name">${main.icon} ${escapeHtml(main.name)}</div>
          <div class="focus-amounts"><strong id="focus-current-amount">${formatMoney(0)}</strong> / ${formatMoney(main.targetAmount)}</div>
          <div class="progress-track"><div class="progress-fill ${p >= 100 ? "complete" : ""}" style="width:0%" data-final="${p}%"></div></div>
      ...
      const moment = momentumMessage(p);
      momentumHtml = `
        <div class="momentum-card card-enter" style="animation-delay:0.05s">
          <div class="momentum-icon">${moment.icon}</div>
          <div>
            <div class="momentum-title">Seu momento</div>
            <div class="momentum-text">${moment.text}</div>
          </div>
        </div>`;
    }
    ...
    const insightsHtml = insights.length
      ? `<div class="insights-list">${insights.map((i, idx) => `<div class="insight-item card-enter" style="animation-delay:${0.05 * idx}s">${i}</div>`).join("")}</div>`
      : "";
    ...
    container.innerHTML = `
      ...
      <div class="summary-grid">
        <div class="summary-card card-enter"><div class="label">💰 Total das metas</div><div class="value" id="stat-total-metas">${formatMoney(0)}</div></div>
        <div class="summary-card card-enter" style="animation-delay:0.04s"><div class="label">✅ Total conquistado</div><div class="value accent" id="stat-total-conquistado">${formatMoney(0)}</div></div>
        <div class="summary-card card-enter" style="animation-delay:0.08s"><div class="label">📈 Progresso geral</div><div class="value" id="stat-progresso-geral">0%</div></div>
        <div class="summary-card card-enter" style="animation-delay:0.12s"><div class="label">🏁 Metas concluídas</div><div class="value">${summary.concluidas} / ${summary.total}</div></div>
      </div>
      ${focusHtml}
      ${momentumHtml}
    ...
    // Animações de entrada: contagem numérica e preenchimento da barra de progresso
    animateCountUp(qs("#stat-total-metas"), summary.totalMetas, { isMoney: true });
    animateCountUp(qs("#stat-total-conquistado"), summary.totalConquistado, { isMoney: true });
    animateCountUp(qs("#stat-progresso-geral"), clampPct(summary.progressoGeral), { decimals: 1, suffix: "%" });
    if (main) {
      animateCountUp(qs("#focus-current-amount"), main.currentAmount, { isMoney: true });
    }
    animateProgressBars(container);
  }

  /** Anima um número de 0 até o valor final (ease-out), usado nos cartões de resumo. */
  function animateCountUp(el, endValue, { isMoney = false, decimals = 0, suffix = "", duration = 700 } = {}) {
    if (!el) return;
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      el.textContent = isMoney ? formatMoney(endValue) : endValue.toFixed(decimals) + suffix;
      return;
    }
    ...
    function frame(now) {
      ...
      el.textContent = isMoney ? formatMoney(value) : value.toFixed(decimals) + suffix;
      if (t < 1) requestAnimationFrame(frame);
    }
    requestAnimationFrame(frame);
  }

  /** Ativa a transição de largura das barras de progresso após a inserção no DOM. */
  function animateProgressBars(root) {
    const bars = qsa(".progress-fill[data-final]", root);
  ...
  function renderGoalCard(goal, index = 0) {
    const p = clampPct(pct(goal.currentAmount, goal.targetAmount));
    const remaining = Math.max(0, goal.targetAmount - goal.currentAmount);
    const dr = daysRemaining(goal.deadline);
    const photoHtml = goal.imageData
      ? `<div class="goal-card-photo"><img src="${goal.imageData}" alt="Foto do objetivo: ${escapeHtml(goal.name)}" loading="lazy" /></div>`
      : "";
    return `
      <div class="goal-card card-enter ${goal.completedAt ? "completed" : ""} ${goal.imageData ? "has-photo" : ""}" data-goal-id="${goal.id}" style="animation-delay:${Math.min(index * 0.05, 0.3)}s">
        ${photoHtml}
  ...
  function renderMetas() {
    ...
    container.innerHTML = `
      ...
      ${
        goals.length
          ? `<div class="goals-grid mt-16">${goals.map((g, i) => renderGoalCard(g, i)).join("")}</div>`
          : `<div class="empty-state">
              <div class="emoji">🔍</div>
  ...
  function openGoalForm(goalId = null) {
    ...
    pendingPhotoData = goal?.imageData || null;
      ...
      <div class="modal-sheet">
        ...
        <form id="goal-form" novalidate>
          <div class="form-group">
            <label>📸 Foto do objetivo</label>
            <div class="photo-upload" id="photo-upload">
              ${photoPreviewMarkup(pendingPhotoData)}
            </div>
  ...
  function submitGoalForm(existingGoal) {
    ...
    const proceed = () => {
      ...
      const photoToSave = pendingPhotoData;
      let goalId;
      if (existingGoal) {
        ...
        DB.atualizarMeta(existingGoal.id, {
          ...
          isMain,
          icon,
          imageData: photoToSave,
        });
        goalId = existingGoal.id;
        if (!DB.wasLastSaveSuccessful()) {
          // sem espaço suficiente no dispositivo — mantém a meta, mas sem a foto
          DB.atualizarMeta(existingGoal.id, { imageData: null });
          toast("A meta foi salva, mas não havia espaço suficiente para a foto.", "error");
        } else {
        ...
        goalId = created.id;
        if (!DB.wasLastSaveSuccessful()) {
          DB.atualizarMeta(created.id, { imageData: null });
          toast("A meta foi criada, mas não havia espaço suficiente para a foto.", "error");
        } else {
    ...
    overlay.innerHTML = `
      <div class="modal-sheet">
        ...
        <div class="detail-hero">
          ${goal.imageData ? `<div class="detail-hero-photo"><img src="${goal.imageData}" alt="Foto do objetivo: ${escapeHtml(goal.name)}" /></div>` : ""}
          <span class="icon">${goal.icon}</span>
          ...
          <div class="progress-track mt-16"><div class="progress-fill ${p >= 100 ? "complete" : ""}" style="width:0%" data-final="${p}%"></div></div>
                  ...
                  .map(
                    (c) => `<div class="history-item">
                      ...
                      <div class="history-body">
                        ...
                        <div class="history-meta">${formatEntryMeta(c.date)}</div>
                      </div>
                    </div>`
                  )
                  .join("")}
  ...
  function renderAnalises() {
    ...
    container.innerHTML = `
      ...
      <div class="card chart-card">
        <h3>Comparação entre metas</h3>
        <canvas id="chart-comparacao" height="200"></canvas>
      </div>
    ...
    // Comparação entre metas (valor atual de cada uma, lado a lado)
    const comparisonItems = goals.map((g, i) => ({
      label: g.name,
      value: g.currentAmount,
      color: g.completedAt ? getCssVar("--accent-2") : colorForIndex(i),
    }));
    Charts.comparisonBars(qs("#chart-comparacao"), comparisonItems);
            ...
            ? grouped
              ...
              ${group.items
                .map(
                  ...
                  <div class="history-icon">${e.goalIcon}</div>
                  <div class="history-body">
                    <div class="history-amount">+ ${formatMoney(e.amount)} <span class="text-tertiary" style="font-weight:400;">— ${escapeHtml(e.goalName)}</span></div>
                    ${e.note ? `<div class="history-note">${escapeHtml(e.note)}</div>` : ""}
                    <div class="history-meta">${formatEntryMeta(e.date)}</div>
                  </div>
                </div>`
                )
                .join("")}
  ...
  const ACHIEVEMENT_GROUP_LABELS = {
    progresso: "🎯 Marcos de progresso",
    valor: "🏅 Valor acumulado",
    constancia: "🔥 Constância",
    metas: "🏆 Metas concluídas",
  };
  ...
  function renderConquistas() {
    ...
    const groups = ["progresso", "valor", "constancia", "metas"];
    let cardIndex = 0;

    container.innerHTML = `
      <h2 style="font-size:20px;">Conquistas</h2>
      <p class="text-secondary mt-8" style="font-size:13.5px;">${unlockedCount} de ${achievements.length} badges desbloqueados</p>

      ${groups
        .map((groupId) => {
          const items = achievements.filter((a) => a.group === groupId);
          if (!items.length) return "";
          const html = items
            .map((a) => {
              const delay = Math.min(cardIndex++ * 0.04, 0.4);
              return `
              <div class="achievement-card card-enter ${a.unlocked ? "unlocked" : ""}" style="animation-delay:${delay}s">
            ...
            .join("");
          return `<div class="section-title">${ACHIEVEMENT_GROUP_LABELS[groupId]}</div><div class="achievements-grid">${html}</div>`;
        })
        .join("")}
        ...
        completedGoals.length
          ...
          <div class="goals-grid mt-16">
            ${completedGoals
              .map(
                (g, i) => `
              <div class="goal-card completed card-enter ${g.imageData ? "has-photo" : ""}" style="animation-delay:${Math.min(i * 0.05, 0.3)}s">
                ${g.imageData ? `<div class="goal-card-photo"><img src="${g.imageData}" alt="Foto do objetivo: ${escapeHtml(g.name)}" loading="lazy" /></div>` : ""}
                <div class="goal-card-body">
